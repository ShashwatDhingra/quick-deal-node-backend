...
class AuthService {
  ...
  async signup(name, email, password) {
    try {
      ...
      if (result && token) {
        ...
        return getResponse(201, true, "Account Created Successfully", {
          token: token,
        });
      }
    ...
    try {
      ...
      if (!isPassValid) {
        return getResponse(
          401,
          false,
          "Wrong password buddy! Cross check it",
          null,
          "Wrong password buddy! Cross check it"
        );
      }
  ...
  async forgetPassword(email) {
    try {
      ...
      const result = authUtils.sendResetPasswordMail(
        email,
        user.name,
        generatePin
      );
  ...
  async verifyResetPin(email, pin) {
    try {
      ...
      const isValid = await userModel.findOne({
        email,
        resetPin: pin,
        resetPinExpiration: { $gt: Date.now() },
      }); // another way to verify the pin weather it expire, using $gt i.e. query operator
